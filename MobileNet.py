...
class InvertedResidual(nn.Module):
    ...
    def forward(self, x):
        ...
        # Otherwise, return the output of passing the provided input through sequentia
...
class MobileNet(nn.Module):
    def __init__(self, num_classes=18, width_mult=1.0, dropout_prob=0.2):
        ...
        for number_of_input_channels, number_of_output_channels, stride, expansion_ratio in configuration:
            scaled_number_of_input_channels = int(number_of_input_channels * width_mult)
            scaled_number_of_output_channels = int(number_of_output_channels * width_mult)
            list_of_layers.append(
                InvertedResidual(
        ...
        self.final_convolution = nn.Sequential(
            nn.Conv2d(
                in_channels=int(320 * width_mult),
